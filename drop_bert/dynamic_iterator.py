...
class DynamicIterator(BasicIterator):
    ...
    def __init__(
        ...
        if not os.path.exists('%s/experiments' % ROOT_DIR):
            os.mkdir('%s/experiments' % ROOT_DIR)
        if not os.path.exists('%s/experiments2' % ROOT_DIR):
            os.mkdir('%s/experiments2' % ROOT_DIR)
