...
@DatasetReader.register("drop_org")
class DropReaderOrg(DatasetReader):
    """
    ...
    def find_valid_spans(passage_tokens: List[Token],
        ...
        for answer_text in answer_texts:
            if len(answer_text.strip()) == 1:
                answer_tokens = answer_text.lower().strip().split()
            else:
            ...
            for span_start in word_positions[answer_tokens[0]]:
                ...
                while answer_index < num_answer_tokens and span_end + 1 < len(normalized_tokens):
                    ...
                    stripped_answer = answer_tokens[answer_index].strip()
                    if len(stripped_answer) != 1:
                        stripped_answer = stripped_answer.strip(STRIPPED_CHARACTERS)
                    if stripped_answer == token:
                        answer_index += 1
                        span_end += 1
