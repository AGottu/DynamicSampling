...
class NumericallyAugmentedBERT(Model):
    ...
    def __init__(self, 
        ...
        model = BertModel.from_pretrained(bert_pretrained_model).state_dict()
        # config = BertConfig.from_dict(bert_config_path)

        self.encoder = encoder
        for encoder_param_name, encoder_param_value in self.encoder.state_dict().items():
            if 'pal' not in encoder_param_name:
                encoder_param_value.data.copy_(model["encoder." + encoder_param_name])

        self.tokenizer = BertTokenizer.from_pretrained(bert_pretrained_model)
        bert_dim = self._text_field_embedder.get_output_dim()
    ...
    def forward(self,  # type: ignore
        ...
        embedded_question_and_passage = self._text_field_embedder(question_passage)

        batch_size = embedded_question_and_passage.size(0)

        extended_attention_mask = pad_mask.unsqueeze(1).unsqueeze(2)
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype)  # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0

        encoded_layers = self.encoder(embedded_question_and_passage, extended_attention_mask,
                                                            output_all_encoded_layers=False)
        bert_out = encoded_layers[-1]
        ...
        if answer_as_passage_spans is not None or answer_as_question_spans is not None \
            ...
            for answering_ability in self.answering_abilities:
                if answering_ability == "passage_span_extraction":
                    ...
                    log_marginal_likelihood_list.append(impossible_answer.type_as(passage_out))
                else:
    ...
    def _count_log_likelihood(self, answer_as_counts, count_number_log_probs):
        ...
        #print(answer_as_counts)
        #print(count_number_log_probs)
        # Shape: (batch_size, # of count answers)
        ...
        #print(log_likelihood_for_counts)
        # For those padded spans, we set their log probabilities to be very small negative value
        ...
        #print(log_marginal_likelihood_for_count)
    ...
    def _base_arithmetic_log_likelihood(self,
        ...
        # Shape: (batch_size, # of numbers in the passage, # of combinations)
        #print(number_sign_log_probs.size())
        #print(gold_add_sub_signs)
