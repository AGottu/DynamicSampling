...
class BERTSelfAttention(nn.Module):
    def __init__(self, config, multi_params=None):
        ...
        self.dropout = nn.Dropout(config.attention_probs_dropout_prob).to(device)
...
class BERTPals(nn.Module):
    def __init__(self, config, extra_dim=None):
        ...
        self.dense1 = nn.Linear(config.hidden_size, config.num_task_layers).to(device)
        self.pals = [BERTSinglePals(config) for i in range(config.num_task_layers)] # Keep multiple PALs per layer with soft weights
        self.config = config
        self.dropout = nn.Dropout(config.hidden_dropout_prob).to(device)

    def forward(self, hidden_states, attention_mask=None):
        pal_scores = self.dense1(hidden_states)
        pal_soft_weights = torch.unsqueeze(nn.Softmax(dim=-1)(pal_scores), dim=3)
        #pal_soft_weights = self.dropout(pal_soft_weights)
        pal_outputs = torch.stack([pal(hidden_states, attention_mask) for pal in self.pals], dim=3)
        output = torch.squeeze(torch.matmul(pal_outputs, pal_soft_weights), dim=3)
        return output

################## Ananth #######################


class BERTSinglePals(nn.Module):
    def __init__(self, config, extra_dim=None):
        super(BERTSinglePals, self).__init__()
        # Encoder and decoder matrices project down to the smaller dimension
        self.aug_dense = nn.Linear(config.hidden_size, config.hidden_size_aug).to(device)
        self.aug_dense2 = nn.Linear(config.hidden_size_aug, config.hidden_size).to(device)
...
class BERTModel(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, i=0, output_all_encoded_layers=True):
